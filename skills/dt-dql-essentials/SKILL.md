...
| `contains(toLowercase(field), "err")` | `contains(field, "err", caseSensitive: false)` | Don't wrap in `lower()` for case-insensitive matching. `contains()` has a built-in `caseSensitive` parameter (default `true`); pass it as a named parameter — the positional form `contains(field, "err", false)` is rejected. |

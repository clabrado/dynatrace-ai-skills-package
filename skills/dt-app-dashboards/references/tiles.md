...
**fieldMapping.timestamp:** Always use `"timeframe"` for BOTH `timeseries` and
`makeTimeseries` queries. Do NOT use `"interval"` — it is a scalar duration string
(e.g. `"60000000000"` nanoseconds), not a timestamp array. The dashboard UI will
reject it as an invalid x-axis field.
